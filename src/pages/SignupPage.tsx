...
import { ArrowLeft, User, MessageSquare, ShieldCheck, Loader2 } from 'lucide-react';
    ...
    <div className="h-full flex flex-col max-w-md mx-auto px-4">
      ...
      <form onSubmit={handleSignup} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button 
            ...
            disabled={isLoading || !!socialLoading}
            className="flex items-center justify-center gap-2 py-3 border border-gray-200 rounded-2xl hover:bg-gray-50 transition-all font-bold text-sm disabled:opacity-50"
          >
          ...
          >
            {socialLoading === 'apple' ? (
              ...
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.06.74.74 0 1.93-.88 3.43-.71 1.51.17 2.64.79 3.32 1.88-3.05 1.77-2.54 5.89.56 7.14-.61 1.51-1.37 2.97-2.37 3.92zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
              </svg>
            )}
            Apple
          </button>
      ...
      <div className="mt-auto py-8 text-center">
        <p className="text-gray-500">
          ...
          <button onClick={() => navigate('/login')} className="text-health-green font-bold hover:underline">Login</button>
        </p>
      </div>
    </div>
  );
